...
for scanNoStart in range(638938,639138):
    ...
    if abs(oldEnergy - rWD.get_nexus_meta("/pgm_energy/pgm_energy"))>0.5:
        ...
        f = open("output\\sample=%.1f_E=%.1f.dat" %(oldPos,oldEnergy), 'w+')
        ...
        y2 = rWD.get_nexus_data("/mcs17/data")
        y = y/y0
        y1 = y1/y0
        y2 = y2/y0
        ...
        rWD.read_nexus_data(folder, scanNoStart)
        yNew = rWD.get_nexus_data("/mcs19/data")/rWD.get_nexus_data("/mcs16/data")
        xNew = rWD.get_nexus_data("/energy/energy")
        y = Rd.average_w_corr(x, xNew, y, yNew)
    
        y1New = rWD.get_nexus_data("/mcs18/data")/rWD.get_nexus_data("/mcs16/data")
        xNew = rWD.get_nexus_data("/energy/energy")
        y1 = Rd.average_w_corr(x, xNew, y1, y1New)
        
            
        y2New = rWD.get_nexus_data("/mcs17/data")/rWD.get_nexus_data("/mcs16/data")
        xNew = rWD.get_nexus_data("/energy/energy")
        y2 = Rd.average_w_corr(x, xNew, y2, y2New)
